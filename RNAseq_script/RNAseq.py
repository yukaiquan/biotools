...
def run_deg(matrix: str, deg_method: str, DEGgroup: str, out: str) -> int:
    cmd = f'perl {DEG} --matrix {matrix} --method {deg_method} --samples_file {DEGgroup} --output {out}'
    p = Popen(cmd, shell=True)
    return_code = p.wait()  # 等待子进程结束，并返回状态码；
    return return_code
...
def argv_init():
    arg = argparse.ArgumentParser(description='RNA-seq pipeline')
    arg.add_argument('-s', '--sample_list', help='sample_list')
    arg.add_argument('-r', '--ref_genome', help='ref_genome')
    arg.add_argument('-i', '--star_index', help='star_index')
    arg.add_argument('-g', '--gtf', help='gtf')
    arg.add_argument('-t', '--threads', help='threads', default=16)
    arg.add_argument('-k', '--gatk', help='gatk', default=False)
    args = arg.parse_args()
    return args
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=parallel) as executor:
        for outname1, [fastq1, fastq2, NIL, _] in sample_dict.items():
            ...
            print(f'{fastp_outdir}/out_{fastq1} fastp start!')
            if not os.path.exists(f'{fastp_outdir}/out_{fastq1}'):
    ...
    for outname1, [fastq1, fastq2, NIL, _] in sample_dict.items():
        # 文件存在且文件大于0时,跳过
        if os.path.exists(f'{star_outdir}' + '/' + f'{outname1} ' + 'Aligned.sortedByCoord.out.bam') and os.path.getsize(f'{star_outdir}' + '/' + f'{outname1} ' + 'Aligned.sortedByCoord.out.bam') > 0:
    ...
    # # 全部样本的 迭代字典值
    with open(f'{feature_outdir}/all.txt', 'w') as f:
        for NIL, sample_list in NIL_dict.items():
            for sample in sample_list:
                f.write(f'{feature_outdir}/{sample}.count\n')
    # # 新建目录保存结果
    ...
    for NIL, sample_list in NIL_dict.items():
        if run_merge_featurecounts(f'{NIL}', f'{feature_outdir}/{NIL}.txt') == 0:
            ...
            sys.exit(1)
    # # 全部样本的
    if run_merge_featurecounts(f'all', f'{feature_outdir}/all.txt') == 0:
        # 移动结果保存
        mv_file(f'all.*', f'{matrix_outdir}/')
        print(f'all merge Done !')
    else:
        print('Error')
        sys.exit(1)
    # 创建差异表达分析文件
    with open(f'{matrix_outdir}/DEG.txt', 'w') as f:
        for outname1, [fastq1, fastq2, NIL, Phenotype] in sample_dict.items():
            f.write(f'{Phenotype}\t{outname1}_{NIL}_{Phenotype}\n')
    # 差异表达分析
    if run_deg(f'{matrix_outdir}/all.counts.matrix', 'edgeR', f'{matrix_outdir}/DEG.txt', 'edgeR') == 0:
        print('edgeR done!')
    else:
        print('edgeR error!')
    if run_deg(f'{matrix_outdir}/all.counts.matrix', 'DESeq2', f'{matrix_outdir}/DEG.txt', 'DESeq2') == 0:
        print('DESeq2 done!')
    else:
        print('DESeq2 error!')
    if gatk:
