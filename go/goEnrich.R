...
if (org_db == "org.Asanfensan.eg.db") {
  ...
  org_db <- "org.Asanfensan.eg.db"
} else if (org_db == "org.Aot3098v2.eg.db") {
  suppressMessages(library(org.Aot3098v2.eg.db, lib.loc = R_library))
  print("org.Aot3098v2.eg.db is loaded!")
  org_db <- "org.Aot3098v2.eg.db"
} else if (org_db == "org.Asang.eg.db") {
  suppressMessages(library(org.Asang.eg.db, lib.loc = R_library))
  print("org.Asang.eg.db is loaded!")
  org_db <- "org.Asang.eg.db"
} else {
  # 终止程序
...
gene <- read_tsv(input_gene_list, col_names = F, show_col_type = FALSE)
geneid <- gene$X1
ego_BP <- enrichGO(
  OrgDb = org_db,
  keyType = "GID",
...
go_enrich_df <- data.frame(
  ...
  gene = c(ego_result_BP$geneID, ego_result_CC$geneID, ego_result_MF$geneID),
  GeneRatio = c(ego_result_BP$GeneRatio, ego_result_CC$GeneRatio, ego_result_MF$GeneRatio),
  BgRatio = c(ego_result_BP$BgRatio, ego_result_CC$BgRatio, ego_result_MF$BgRatio),
  qvalue = c(ego_result_BP$qvalue, ego_result_CC$qvalue, ego_result_MF$qvalue)
)
go_enrich_df <- unique(go_enrich_df) %>% mutate(qvalue = ifelse(is.na(qvalue), 1, qvalue))
...
all_BP <- as.data.frame(ego_BP)
# 最后添加一列biological process
all_BP$type <- "biological process"
all_CC <- as.data.frame(ego_CC)
all_CC$type <- "cellular component"
all_MF <- as.data.frame(ego_MF)
all_MF$type <- "molecular function"
# 合并三个表
